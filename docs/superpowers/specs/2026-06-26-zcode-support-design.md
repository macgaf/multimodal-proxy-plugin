...
4. **ZCode 没有 `plugin add` 命令**，只有 `zcode plugins [list|enable <id>|disable <id>]`。插件注册机制：
   ...
   - 在 `~/.zcode/cli/config.json` 的 `plugins.enabledPlugins` 写 `"<plugin>@<marketplace>": true`（本机已有先例：`superpowers@zcode-plugins-official`）。注意 ZCode 的 config schema 为嵌套结构 `{"plugins": {"enabledPlugins": {...}}}`，非顶层 `enabledPlugins`。
