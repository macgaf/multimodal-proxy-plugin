...
4. **ZCode 没有 `plugin add` 命令**，只有 `zcode plugins [list|enable <id>|disable <id>]`。插件发现机制（逆向 `zcode.cjs` 的 `xso()`/`yso()`/`Tso()` 确认）：
   - 官方插件来自硬编码的 `Ole` 定义数组（每个带 `rootCandidates` 相对路径），在搜索根 `[app argv 目录, __dirname, process.cwd()]` 下找 `.zcode-plugin/plugin.json`。
   - **第三方插件唯一入口**：`config["plugins"]["dirs"]`——用户显式声明的插件目录绝对路径列表。每个 dir 直接作为 `rootPath`，marketplace 标记为 `"inline"`，`defaultEnabled=true`（默认启用，无需在 `enabledPlugins` 里显式写）。
   - `marketplace.json` / `cache/` / `enabledPlugins` 是官方插件安装后的缓存记录，**不是**第三方插件的发现机制——光写这些不会被 ZCode 扫描到。
...
| `scripts/zcode_register.py` | **新增** | ZCode 注册逻辑（Python，避免 bash 拼 JSON）：把插件根路径 upsert 进 `~/.zcode/cli/config.json` 的 `plugins.dirs`。被 install.sh 调用。CLI 接口：`zcode_register.py --plugin-root <PLUGIN_ROOT>`（`--cli-root` 可选，默认 `~/.zcode/cli`）。 |

> **实现更正（2026-06-27）**：本 spec 初版误以为 ZCode 通过 `marketplace.json` + `cache/` + `enabledPlugins` 发现第三方插件（仿官方插件缓存结构）。逆向 `zcode.cjs` 的 `xso()`/`yso()`/`Tso()` 后确认：官方插件来自硬编码 `Ole` 数组，第三方插件**唯一入口**是 `config["plugins"]["dirs"]`。§2/§3/§4 下文仍保留初版措辞作为设计演进记录，但**实际实现以 `zcode_register.py` 为准**（写 `plugins.dirs`，不写 marketplace/cache/enabledPlugins）。
...
2. `bash scripts/install.sh --target zcode` 在本机执行成功，`~/.zcode/cli/config.json` 的 `plugins.dirs` 含插件根路径。
